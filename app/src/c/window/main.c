...
static void toggle_status(bool hidden) {
  if (!hidden) {
    update_buffer();
  }
  layer_set_hidden(action_bar_layer_get_layer(action_bar), hidden);
...
static void select_click_handler(ClickRecognizerRef recognizer, void *context) {
  toggle_status(!layer_get_hidden(action_bar_layer_get_layer(action_bar)));
}
...
void main_load(Window* window) {
  ...
  toggle_status(global->result != HEALTH_RESULT_QUEST);
}
